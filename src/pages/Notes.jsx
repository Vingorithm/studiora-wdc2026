...
const defaultNotes = [
  {
    ...
    subject: 'Mathematics',
    content: '<p>Key methods: substitution, integration by parts, partial fractions.</p><p>Remember: ∫uv dx = u∫v dx – ∫(u′∫v dx)dx. Practice the LIATE rule for choosing u in integration by parts.</p>',
    tags: ['calculus', 'exam'],
    highlighted: true,
    checklist: [
  ...
  },
  {
    ...
    subject: 'English',
    content: "<p>Major themes: mortality (<em>To be or not to be</em>), revenge vs. justice, corruption in the Danish court.</p>",
  ...
  },
  {
    ...
    subject: 'Physics',
    content: '<p><strong>1st Law:</strong> An object at rest stays at rest unless acted upon by a net force.</p><p><strong>2nd Law:</strong> F = ma. <strong>3rd Law:</strong> For every action there is an equal and opposite reaction.</p>',
    tags: ['mechanics', 'lab'],
    highlighted: false,
    checklist: [],
  ...
  },
  {
    ...
    subject: 'Marketing',
    content: '<p><strong>Product, Price, Place, Promotion.</strong></p><p>For group project: focus on digital promotion strategies. Research Nike\'s 4Ps as case study.</p>',
...
function NoteCard({ note, onView, onDelete }) {
  ...
  const date = new Date(note.createdAt || note.updatedAt).toLocaleDateString('en-US', {
  ...
  return (
    <div className="group relative flex flex-col bg-white rounded-2xl shadow-card border border-slate-100 overflow-hidden transition-all duration-300 hover:shadow-soft hover:-translate-y-1">
      ...
      <div className="flex flex-col flex-1 p-5">
        {note.subject && (
          <span className={`inline-block text-xs font-semibold font-inter px-2.5 py-1 rounded-lg mb-3 self-start text-white bg-gradient-to-r ${accent.bar}`}>
            {note.subject}
          </span>
        )}
        ...
        {note.tags && note.tags.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-4">
            ...
            {note.tags.length > 3 && (
              <span className="text-xs text-slate-400 font-inter">+{note.tags.length - 3}</span>
            )}
          </div>
        )}
      ...
      {note.highlighted && (
        <div className="absolute top-3 right-3">
          <span className="text-sm" title="Highlighted">✨</span>
        </div>
      )}
    </div>
  )
...
function EmptyState({ onNew }) {
  return (
    <div className="flex flex-col items-center justify-center py-24 text-center animate-fade-in">
      ...
      <h3 className="text-xl font-bold font-poppins text-darkText mb-2">No notes yet</h3>
      <p className="text-slate-400 font-inter text-sm max-w-xs mb-8 leading-relaxed">
        Start capturing your ideas. Notes help you retain knowledge and ace your exams.
      </p>
...
export default function Notes() {
  ...
  const saveNotes = (n) => { localStorage.setItem(STORAGE_KEY, JSON.stringify(n)); setNotes(n) }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-400 font-inter">
            {notes.length} note{notes.length !== 1 ? 's' : ''} across {allSubjects.length} subject{allSubjects.length !== 1 ? 's' : ''}
          </p>
        </div>
        <button
          onClick={() => navigate('/new-note')}
          className="inline-flex items-center gap-2 font-semibold font-inter text-sm text-white px-6 py-3 rounded-xl shadow-soft hover:shadow-glow hover:-translate-y-0.5 transition-all duration-300 active:scale-95"
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        ...
        {filterTag && (
          <button onClick={() => setFilterTag('')} className="flex items-center gap-1.5 text-sm text-primary bg-primary/10 px-4 py-2 rounded-xl font-inter font-medium hover:bg-primary/20 transition-colors">
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {allTags.map(t => (
            <button
              ...
              style={filterTag === t ? { background: 'linear-gradient(135deg, #5D8BF4, #C4B5FD)' } : {}}
            >
              #{t}
            </button>
          ))}
      ...
      {filtered.length === 0 && !search && !filterTag && !filterSubject ? (
        <EmptyState onNew={() => navigate('/new-note')} />
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 text-slate-400">
          <p className="text-4xl mb-3">🔍</p>
          <p className="font-poppins font-semibold text-slate-500">No notes match your search</p>
          <button onClick={() => { setSearch(''); setFilterTag(''); setFilterSubject('') }} className="mt-4 text-sm text-primary font-semibold font-inter hover:underline">
            Clear filters
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {filtered.map((note, i) => (
            <div key={note.id} className="animate-slide-up" style={{ animationDelay: `${i * 0.05}s` }}>
              <NoteCard note={note} onView={setViewNote} onDelete={handleDelete} />
            </div>
          ))}
          <button
            onClick={() => navigate('/new-note')}
          ...
          >
            <div className="w-10 h-10 rounded-xl border-2 border-dashed border-slate-200 group-hover:border-primary/40 flex items-center justify-center mb-3 text-xl transition-colors">+</div>

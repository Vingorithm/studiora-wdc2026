...
// Priority → Tailwind color class for calendar blocks
const PRIORITY_COLORS = {
  High: "bg-danger/15 text-red-500 border-l-2 border-red-400",
  Medium: "bg-accent/20 text-orange-500 border-l-2 border-orange-400",
  Low: "bg-success/15 text-emerald-600 border-l-2 border-emerald-500",
  default: "bg-primary/20 text-primary border-l-2 border-primary",
};
...
function TaskModal({ task, onSave, onClose }) {
  ...
  return (
    <div
      ...
      onClick={onClose}
      aria-hidden="true" // backdrop is decorative; the dialog below carries semantics
    >
      {/* role="dialog" + aria-modal tells screen readers to restrict virtual cursor */}
      <div
        ref={modalRef}
        role="dialog"
        aria-modal="true"
...
export default function Tasks() {
  ...
  const handleDelete = (id) => {
    ...
    save(remaining);
    toast(
      `"${deletedTask?.title || "Task"}" deleted`,
      "info",
      5000,
  ...
  // Build calendar blocks from real tasks (deadline = which day column)
  const weekDates = getWeekDates();
  const calendarBlocks = tasks
    .filter((t) => t.deadline && t.status !== "Done")
    .flatMap((task) => {
      const deadline = new Date(task.deadline + "T00:00:00"); // avoid TZ shift
        ...
        <div className="flex gap-2">
          ...
          >
            <button
              ...
              className={`px-4 py-2 text-sm font-inter font-medium transition-colors flex items-center gap-1 ${view === "board" ? "bg-primary text-white" : "text-slate-500 hover:bg-slate-50"}`}
            >
              <FaClipboard /> Board
            </button>
            <button
              ...
              className={`px-4 py-2 text-sm font-inter font-medium transition-colors flex items-center gap-1 ${view === "calendar" ? "bg-primary text-white" : "text-slate-500 hover:bg-slate-50"}`}
            >
...
<<<<<<< HEAD
        /* ── Calendar view — Fix 3: real task data ── */
...
<<<<<<< HEAD
                ...
                return (
                  <div
                    key={d}
                    className={`text-center py-2 rounded-xl ${isToday ? "bg-primary text-white" : "text-slate-500"}`}
                  >
...
<<<<<<< HEAD
              {HOURS.map((h) => (
                <div
                  key={h}
                  className="grid grid-cols-8 gap-1 border-t border-slate-50"
                >
                  <div className="text-xs text-slate-300 font-inter py-3 px-2">
                    {h}:00
                  </div>
                  {DAYS.map((d, di) => {
                    ...
                    return (
                      <div
                        key={d}
                        className="relative h-10 border-l border-slate-50 hover:bg-primary/5 transition-colors rounded-r-lg"
                      >
                        {block && (
                          <div
                            className={`absolute inset-x-1 top-0.5 rounded-lg px-2 py-1 text-xs font-semibold font-inter truncate ${block.color}`}
...
<<<<<<< HEAD
            ...
            {/* Legend when no blocks */}
            {calendarBlocks.length === 0 && (
              <div className="text-center py-8 text-slate-300">
                <p className="text-2xl mb-2">📅</p>
                <p className="font-inter text-sm">
                  No upcoming deadlines this week. Add tasks with deadlines to
                  see them here.
                </p>
              </div>
            )}
